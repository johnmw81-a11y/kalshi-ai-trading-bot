...
# --- NCAA HOOPS CONFIG ---
# Target: Michigan to beat Tennessee (Elite Eight)
TARGET_TICKER = "NCAA-260329-MICH-TENN" 
MAX_POSITION_DOLLARS = 45   
TRADE_AMOUNT_DOLLARS = 20   

# --- THE PRO SHIELDS (Basketball is swingy!) ---
BUY_PRICE_LIMIT = 50        # 🛑 Wait for a tie game or slight deficit to buy in cheap
HARVEST_PRICE = 85          # 🌾 Take profit on a big scoring run
STOP_LOSS_PRICE = 25        # 🚨 Bail out if they go down by 15+ points
...
def run_ncaa_sniper():
    print(f"🏀 NCAA SNIPER WAKING UP: {time.strftime('%X')} CT")
    ...
    if current_value_dollars >= MAX_POSITION_DOLLARS:
        print(f"🛡️ Max position limit reached. Holding steady.")
        return
